...
- Records anonymous QR clicks through the endpoint configured in `analytics-config.js`.
...
- `analytics-config.js` - analytics endpoint configuration.
...
For OAKO, `analytics-config.js` points to the Firebase `trackQrClick` endpoint.
That endpoint stores anonymous aggregates in Firestore and the OAKO Admin
Analytics tab displays daily, weekly, monthly, and top-link QR counts.

To use a different backend, change `window.QR_ANALYTICS_CONFIG.endpoint` in
`analytics-config.js`. If the endpoint is empty, QR generation still works but
